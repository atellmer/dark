...
type GetKey<T> =  (item: T) => string | number | null;

function useTransitions<T>(items: Array<T>, getKey: GetKey<T>, transitionOptions: TransitionOptions) {
...
function getDiffKeyMap<T>(items: Array<T>, prevItems: Array<T>, getKey: (item: T) => string | number): Record<string, boolean> {
  ...
  for (let i = 0; i < iterations; i++) {
    ...
    const key = item && getKey && getKey(item);
