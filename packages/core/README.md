...
By default, core runs in synchronous mode, however, if Dark understands that it is rendering on the server, it goes into asynchronous mode and can wait for lazy modules to load and asynchronous code to execute if the `useResource` hook is used.
...
```tsx
const { loading, data, error } = useResource(() => fetchUserById(1));
