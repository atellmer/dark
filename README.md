...
A [small application](https://github.com/atellmer/dark/tree/master/examples/server-side-rendering/) demonstrating the capabilities of Dark using `rendering to stream`, `serive-worker`, `router`, `styled components` and `async queries` scores maximum points in Lighthouse.
